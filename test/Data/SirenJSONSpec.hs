...
spec :: Spec
spec =
  describe "application/vnd.siren+json" $
    modifyMaxSize (const 25) $
      do
        describe "properties" $
          context "fromJust . decode . encode == normalize" $
            do
              prop "Field" (fromJust . decode . encode <=> normalize :: Field -> Bool)
        ...
        describe "href" $
          context "decode absolute URIs only" $
            do
              it "Link" $ (decode mLink_RelativeHref :: Maybe Link) `shouldBe` Nothing
        ...
        describe "JSON Missing Keys" $
          do
            context "decode minimal JSON strings" $
              do
                it "Field" $ isJust (decode mField :: Maybe Field)
